...
## How does it work ?
The function `erlarg:parse/2` will transform a list of arguments into a structured data.
- `Args`: A list of arguments (generaly what's given to `main/1`)
- `Spec`: A spec that describe how the arguments should be parsed
...
```erlang
main(Args) ->
    Spec = {any, [erlarg:opt({"-l", "--limit"}, limit, int),
                  erlarg:opt({"-f", "--format"}, format, binary),
                  erlarg:opt("-o", file, string),
                  erlarg:opt("-", stdin),
                  erlarg:opt({"-m", "--max"}, max, float)
                 ]}
    {ok, {Result, RemainingArgs} = erlarg:parse(Args, Spec),
    ...
```
In this example, `parse` will returns:
```erlang
 % Result
[{limit, 20},
 {max, 0.25},
...
```
Here, we explicitly ask the parser to handle two arguments, the first <u>must</u> be a `string`, the second <u>must</u> be an `int`.  
...
["hello", 3]. % Result
```
To avoid this probem, you can give "name" to the parsed parameters with the following syntax:
```erlang
...
## Options
Naming and casting parameters into types is neat, but most program use options. An option is an argument that usually (not always…) starts with dash and has zero or more parameters.  
```bash
...
```
Well… that's quite unreadable… fortunately, you can use a map and `alias`:
```erlang
Defs = #{
...
{ok, {Result, _}} = erlarg:parse(["--opt1", "-a", "abc", "-b", "def", "--opt2"], Spec).
...
That's where the operator `any` shines.  
